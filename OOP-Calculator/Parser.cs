        ...
        public double parse(TextReader text)
        {
            ...
            while ((current = text.Read()) != -1)
            {
                ...
                if (categ == TokenCategory.Ans)
                {
                    if (!ans_popped)
                    {
                        q.Enqueue(new TerminalExpression(answer));
                        ans_popped = true;
                    }
                    continue;
                }
                ...
                {
                    ans_popped = false;
                    if(nextType == TokenCategory.End)
                    {
                        throw new Exception("Invalid use of operator at the end of the line");
                    }
                    else if (prevNumber)
                    {
